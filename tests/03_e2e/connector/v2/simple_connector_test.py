...
from pycti.connector.v2.libs.orchestrator_schemas import WorkflowCreate, RunCreate, ConfigCreate, Config, Workflow, Run, \
    JobStatus, State, Result
...
def get_element(job_status_list: List[JobStatus], config_id: str) -> Optional[JobStatus]:
...
def test_connector_workflow(schedule_server, monkeypatch):
    ...
    stix_worker_config = ConfigCreate(
        ...
        config=StixWorker.StixRunConfig(
            token="18bd74e5-404c-4216-ac74-23de6249d690"
        )
    )
    response = requests.post(f"{schedule_server}/config/", json=stix_worker_config.dict())
    ...
    external_import_config = ConfigCreate(
        ...
        config=TestExternalImport.TestExternalImportRunConfig(
            ip="192.168.14.1"
        )
    )
    response = requests.post(f"{schedule_server}/config/", json=external_import_config.dict())
    ...
    response = requests.post(f"{schedule_server}/workflow/", json=workflow_create.dict())
    ...
    response = requests.post(f"{schedule_server}/workflow/{workflow.id}/run", json=run_create.dict())
    print(response.text)
    assert response.status_code == 201
