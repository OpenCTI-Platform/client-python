...
@pytest.fixture
def opencti_stix2(httpserver):
    # Response to pass Health Check
    token = "Dummy key"
    httpserver.expect_request(
        "/graphql", method="POST", headers={"Authorization": f"Bearer {token}"}
    ).respond_with_json({"data": {"threatActors": {"foo": "bar"}}})
    api_client = OpenCTIApiClient(f"http://{httpserver.host}:{str(httpserver.port)}", token)
    return OpenCTIStix2(api_client)


def test_unknown_type(opencti_stix2, caplog):
...
def test_convert_markdown(opencti_stix2: OpenCTIStix2):
    result = opencti_stix2.convert_markdown(" my <code> is very </special> </code> to me")
...
def test_format_date_without_tz(opencti_stix2: OpenCTIStix2):
