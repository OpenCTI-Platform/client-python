...
class TestConnector:
    ...
    def test_register_simple_connector(self, api_connector, simple_connector):
        ...
        assert test_connector == simple_connector.data()[
            'connector_id'], f"No registered connector with id '{simple_connector.data()['connector_id']}' found"
    ...
    def test_register_basic_connector(self, api_connector, api_client):
        ...
        assert config['connector']['id'] == helper.get_connector().to_input()['input'][
            'id'], f"Connector IDs don't match!"
