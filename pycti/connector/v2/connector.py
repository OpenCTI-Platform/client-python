...
from pycti.connector.v2.libs.orchestrator_schemas import ConnectorCreate, Connector as ConnectorSchema, RunContainer, \
    State, Result, Config, RunUpdate
...
class Connector(object):
    def __init__(self, connector_create: ConnectorCreate):
        ...
        self.environment_config, self.connector_instance, self.connector = self.register_connector(connector_create)
        self.logger = get_logger(self.connector.name, self.base_config.log_level)

        self.logger.info(f"Registered with base_config {self.environment_config}. Connector instance: {self.connector_instance}")

        if self.environment_config['broker']['type'] == "pika":
            self.broker = PikaBroker(self.environment_config['broker'], self.process_broker_message)
            self.broker_thread = threading.Thread(target=self.broker.listen, name="Pika Broker Listen",
        ...
        self.heartbeat = Heartbeat(self.base_config.scheduler, self.connector_instance, self.base_config.log_level, self.environment_config['heartbeat']['interval'])
        self.heartbeat.start()

        # self.api.
        self.logger.info("Started")

    def register_connector(self, connector_create: ConnectorCreate) -> (dict, str, ConnectorSchema):
        connector_create.config_schema = connector_create.config_schema.schema_json()
        response = requests.post(
            url=f"{self.base_config.scheduler}/connector/",
            json=connector_create.dict()
        )
        if response.status_code != 201:
            raise ValueError(f"Got response code {response.status_code} '{response.text}'")
    ...
    def process_broker_message(self, run: RunContainer) -> RunContainer:
        ...
        response_code, response_msg = self.update_status(job.config_id, run.run_id, State.running)
        ...
        try:
            ...
            raise RunException(
                f"Unable to parse config {str(e)}"
            )
        except Exception as e:
            raise RunException(
                f"Run error {str(e)} {traceback.format_exc()}"
            )

        response_code, response_msg = self.update_status(job.config_id, run.run_id, State.finished, Result.success)
        if response_code != 200:
            self.update_status(job.config_id, run.run_id, State.finished, Result.fail)
            raise RunException(
    ...
    def get_config(self, config_id: str) -> Config:
        response = requests.get(
            url=f"{self.base_config.scheduler}/config/{config_id}"
        )
    ...
    def update_status(self, config_id: str, run_id: str, status: State, result: Result = None) -> (int, str):
        ...
        response = requests.put(f"{self.base_config.scheduler}/run/{run_id}", json=update_schema.dict())
...
class Heartbeat(threading.Thread):
    def __init__(self, scheduler: str, connector_instance: str, log_level: str, interval: int):
